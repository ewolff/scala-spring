...
import de.adesso.scalaspring.tx.TransactionManagement

class CustomerDAOTx(dataSource: DataSource) extends TransactionManagement with CustomerDAO {
  
  val jdbcTemplate = new JdbcTemplate(dataSource)

  def save(customer: Customer): Customer = transactional() {
    jdbcTemplate.update(
  ...
  def deleteById(id: Int) = transactional() {
    jdbcTemplate.update("DELETE FROM CUSTOMER WHERE ID=?", id : java.lang.Integer)
  }

  def count(): Int = transactional() { 
    jdbcTemplate.queryForInt("SELECT COUNT(*) FROM CUSTOMER")
  }
  
  def findById(id: Int): Option[Customer] = transactional() {
    val result : java.util.List[Customer] = jdbcTemplate.query(
      ...
      } ): RowMapper[Customer],  id : java.lang.Integer)
    result.headOption
  }
